...
describe('the shipped Web composition', () => {
  ...
  it('supplies the shipped preset set, and only those, from the system root', async () => {
    const listed = await ctx.agentPresets.list()

    expect(listed.map(preset => preset.id).sort()).toEqual(['code', 'cordis', 'minimal', 'minimal-rtk', 'standard'])
