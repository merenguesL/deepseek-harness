...
设置 → 通用的外观行还包含「模型回复输出背景」：一个开关配上产品固定的浅蓝底色（`--dsw-static-deepseek-50` 的值，与用户气泡表面同色），持久化为 `ui-theme.outputTint`（空字符串表示关闭；其余持久化值——已移除调色盘的遗留——在采纳时归一为固定颜色）。快照以 `outputTint` 字段暴露该颜色，[ui-conversation](../ui-conversation/README.zh.md) 用它绘制模型回复的文字输出（深色模式下通过 color-mix 减弱）；思考过程与工具调用头部保持无底色。
