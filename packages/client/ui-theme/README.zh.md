...
设置 → 通用的外观行还包含「模型回复输出背景」：一个开关加预设／自定义调色盘，持久化为 `ui-theme.outputTint`（CSS 颜色；空字符串表示关闭）。快照以 `outputTint` 字段暴露该颜色，[ui-conversation](../ui-conversation/README.md) 用它绘制模型回复的文字输出（深色模式下通过 color-mix 减弱）；思考过程与工具调用头部保持无底色。
