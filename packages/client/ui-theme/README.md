...
The Appearance row in Settings → General also carries the assistant-output tint: a switch plus a preset/custom color palette persisted as `ui-theme.outputTint` (a CSS color; empty disables). The snapshot exposes it as `outputTint`, and [ui-conversation](../ui-conversation/README.md) paints the assistant text output with it (dimmed on the dark palette via color-mix); reasoning and tool-call heads stay untinted.
