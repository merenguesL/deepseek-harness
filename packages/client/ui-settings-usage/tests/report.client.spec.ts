...
function session(id: string, updatedAt: number, usage?: TokenUsage, cwd?: string): SessionSummary {
  const row = {
  ...
  if (usage === undefined) return row as SessionSummary
  return {
    ...row,
    projections: {
      asOfSeq: 1,
...
describe('buildUsageReport', () => {
  it('aggregates tokenUsage rows without changing the session-list contract', () => {
    ...
    const value = buildUsageReport([
      session('alpha', firstAt, {
        uncachedInputTokens: 100, outputTokens: 50, cacheReadTokens: 300, cacheWriteTokens: 10,
      }, '/work/project'),
      session('beta', secondAt, {
        uncachedInputTokens: 20, outputTokens: 30, cacheReadTokens: 0, cacheWriteTokens: 5,
      }, '/work/project'),
    ...
    expect(value.bySession.map(row => String(row.sessionId))).toEqual(['alpha', 'beta', 'empty'])
    expect(value.byWorkspace[0]).toMatchObject({
      path: '/work/project', sessions: 2, totalTokens: 515, calls: 2,
    })
    expect(value.byWorkspace[1]).toMatchObject({
      path: '/work/other', sessions: 1, totalTokens: 0, calls: 0,
    })
    expect(value.series).toEqual([
      expect.objectContaining({ day: dayOf(firstAt), totalTokens: 460, calls: 1 }),
      expect.objectContaining({ day: dayOf(secondAt), totalTokens: 55, calls: 1 }),
    ])
    expect(value.heatmap[new Date(firstAt).getHours()]![new Date(firstAt).getDay()]).toBe(460)
    expect(value.heatmap[new Date(secondAt).getHours()]![new Date(secondAt).getDay()]).toBe(55)
  })

  it('normalizes malformed projection values and leaves missing values empty', () => {
    const value = buildUsageReport([session('bad', 1, {
